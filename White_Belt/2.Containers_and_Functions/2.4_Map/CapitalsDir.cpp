...
В ответ на запрос DUMP выведите
    ...
    При выводе последовательности пары указанного вида необходимо упорядочить по названию страны 
    и разделять между собой пробелом.
...
void ParseReq(std::map<std::string, std::string>& countryDir) {
    std::string req;
    std::cin >> req;
    ...
    if (req == "CHANGE_CAPITAL") {
        ...
        if (countryDir[country].empty()) {
            std::cout << "Introduce new country " << country << " with capital " << new_capital << std::endl;
        } else if (countryDir[country] == new_capital) {
            std::cout << "Country " << country << " hasn't changed its capital" << std::endl;
        } else {
            std::cout << "Country " << country << " has changed its capital from " << countryDir[country] << " to " 
                    << new_capital << std::endl;
        }

        countryDir[country] = new_capital;
    } else if (req == "RENAME") {
        std::string old_country_name, new_country_name;
        std::cin >> old_country_name >> new_country_name;

        if (!countryDir.count(old_country_name) && countryDir.count(new_country_name)) {
            std::cout << "Incorrect rename, skip" << std::endl;
            return;
        } else {
            std::cout << "Country " << old_country_name << " with capital " << countryDir[old_country_name] << 
                " has been renamed to " << new_country_name << std::endl;
        }

        countryDir[new_country_name] = countryDir[old_country_name];
        countryDir.erase(old_country_name);
    } else if (req == "ABOUT") {
        std::string country;
        std::cin >> country;
        ...
        if (countryDir[country].empty()) {
            ...
            countryDir.erase(country);
        } else {
            std::cout << "Country " << country << " has capital " << countryDir[country] << std::endl;
        }
    } else if (req == "DUMP") {
        if (countryDir.empty()) {
            ...
            std::string result;
            for (const auto& [country, capital] : countryDir)
            {
...
int main() {
    int Q;
    std::cin >> Q;
    std::map<std::string, std::string> countryDir;
